...
**Note:** it's not a chess engine but it could be a good starting point for it. It can play the role of a core (regarding move generation and validation) for multiple chess engines running on distributed Erlang nodes, since Binbo is an OTP application itself. In addition, Binbo can communicate with foreign engines that support https://www.chessprogramming.org/UCI[Universal Chess Interface] (UCI).
