...
.where:
* `Pid` is the pid of the game process;
* `Movelist` is a list of all legal moves for the current position. Each element of `Movelist` is a tuple `{From, To}`, where `From` and `To` are starting and target square respectively. The values of `From` and `To` depend on `Movetype`.
...
The elements `{From, To}` of `Movelist` depend on `Movetype` as follows:
