...
==== Quick start playing with engine
...
%% Say, you want to play Black. Let the engine make move for White.
> binbo:uci_play(Pid, #{}).
{ok,continue,<<"e2e4">>}
...
%% It's tour turn now. Let the engine search the best move for you with default options.
%% No move actually done, just hint:
> binbo:uci_bestmove(Pid, #{}).
{ok,<<"g8f6">>}

%% Search the best move at depth 20:
